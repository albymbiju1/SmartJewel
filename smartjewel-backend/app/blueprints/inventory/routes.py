...
def get_item(item_id):
    ...
    # Inject active discount if one applies to this item
    try:
        now = datetime.utcnow()
        base_price = d.get("computed_price") or d.get("price") or 0
        if base_price:
            discount = db.discounts.find_one({
                "active": True,
                "scope": "product",
            ...
            if not discount:
                item_category = (d.get("category") or d.get("metal") or "").strip().lower()
                if item_category:
                    discount = db.discounts.find_one({
                        "active": True,
                        "scope": "category",
            ...
            if discount:
                ...
                d["active_discount"] = {
                    ...
                    "discount_value": dvalue,
                    "discount_amount": round(damount, 2),
                    "discounted_price": round(max(0, base_price - damount), 2),
                }
    except Exception:
        pass  # Discount lookup failure must never break item fetch
